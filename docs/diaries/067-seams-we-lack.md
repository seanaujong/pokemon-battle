...
|---|---|---|---|
| **`data/mods/`** structure for gen-specific overrides at the *data* layer | `gen/simplified/` is a sub-package with subclasses, not parallel data trees | Adding a second non-Gen-V gen as a real format (Gen 1, 4, 7, etc.) | Medium. Pattern is clear; existing subclassing is the seed. |
| **`aliases.ts`** as a first-class data file (e.g. "Mega Mewtwo X" → "mewtwomegax") | Slug aliases live as a Kotlin `Map` inside `SmogonToTargetsMain` | Ingesting a third source whose names disagree with PokeAPI/Smogon (Pokemon Home, Bulbapedia, etc.) | Small. Extract the existing map into a committed `data/aliases.json`. |
