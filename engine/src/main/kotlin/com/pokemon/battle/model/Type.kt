...
enum class Type {
    ...
    companion object {
        ...
        fun effectiveness(
            attacking: Type,
            defending: Type,
        ): Double = CHART[attacking]?.get(defending) ?: 1.0
    }
}
...
enum class Effectiveness {
    ...
    companion object {
        fun from(multiplier: Double): Effectiveness =
            when {
...
fun typeEffectiveness(
    attackingType: Type,
    defendingTypes: List<Type>,
): Double = defendingTypes.fold(1.0) { acc, def -> acc * Type.effectiveness(attackingType, def) }

// Sparse chart — only non-1.0 entries. Missing = neutral (1.0).
private val CHART: Map<Type, Map<Type, Double>> =
    mapOf(
        Type.NORMAL to
            mapOf(
